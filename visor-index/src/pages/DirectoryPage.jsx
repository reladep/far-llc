...
export default function DirectoryPage() {
  ...
  return (
    <div style={{ background: '#F6F8F7', fontFamily: 'DM Sans, sans-serif', minHeight: '100vh' }}>
      ...
      {/* Hero */}
      <div style={{ background: '#0A1C2A', paddingTop: 56 }}>
        <div style={{ maxWidth: 1120, margin: '0 auto', padding: '36px 56px 40px' }}>
          <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase', color: '#2DBD74', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ width: 16, height: 1, background: '#2DBD74', display: 'inline-block' }}/>
            Advisor Directory
          </div>
          <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 34, fontWeight: 700, color: '#fff', letterSpacing: '-0.02em', marginBottom: 6 }}>
            Browse by State
          </div>
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.35)', lineHeight: 1.65, maxWidth: 480, marginBottom: 28 }}>
      ...
      {/* Map section */}
      <div style={{ maxWidth: 1120, margin: '0 auto', padding: '40px 56px 64px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 20 }}>
          <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 22, fontWeight: 700, color: '#0C1810' }}>Browse by State</div>
        ...
        <div style={{ background: '#fff', border: '1px solid #CAD8D0', borderTop: '2px solid #0C1810', padding: '20px 20px 12px', marginBottom: 1 }}>
          ...
          {!loading && !error && (
            <div style={{ position: 'relative' }}>
              <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', display: 'block' }} onMouseLeave={handleMouseLeave}>
                {features.map(f => (
                  <path
                    key={f.id} d={pathGen(f)} fill={getFill(f.id)}
                ...
                {tooltip && (
                  <g transform={`translate(${Math.min(tooltip.x + 16, W - 185)},${Math.max(tooltip.y - 72, 8)})`} style={{ pointerEvents: 'none' }}>
                    <rect width={174} height={62} fill="#0A1C2A" rx={0}/>
                    <rect width={174} height={2} fill="#2DBD74"/>
                    <text x={12} y={20} fontFamily="Cormorant Garamond, serif" fontSize={14} fontWeight={700} fill="white">{tooltip.data.name}</text>
                    <text x={12} y={36} fontFamily="DM Mono, monospace" fontSize={10} fill="#2DBD74">{tooltip.data.firms.toLocaleString()} registered firms</text>
                    <text x={12} y={51} fontFamily="DM Mono, monospace" fontSize={10} fill="rgba(255,255,255,.45)">{tooltip.data.aum} total AUM</text>
                  </g>
                )}
              </svg>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 1, background: '#CAD8D0' }}>
          {[
            ...
            <div key={i} style={{ background: '#fff', padding: '16px 20px' }}>
              <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#5A7568', marginBottom: 6 }}>{cell.label}</div>
              {i < 3 ? (
                <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 20, fontWeight: 700, color: '#0C1810', lineHeight: 1, marginBottom: 3 }}>{cell.val}</div>
              ) : (
                <Link to={selected ? '/search' : '#'} style={{
                  fontFamily: 'Cormorant Garamond, serif', fontSize: 18, fontWeight: 700, display: 'block',
                  color: selected ? '#1A7A4A' : '#5A7568',
                  opacity: selected ? 1 : 0.4,
                  marginBottom: 3, textDecoration: 'none',
                }}>Browse Firms →</Link>
              )}
              <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 9, color: '#5A7568' }}>{cell.sub}</div>
            </div>
          ))}
        </div>
      </div>
